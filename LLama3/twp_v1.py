...
class TrainingArguments(transformers.TrainingArguments):
    ...
    model_max_length: int = field(
        default=4096,  # Llama 3 supports longer sequences
        metadata={
            "help": "Maximum sequence length. Sequences will be right padded (and possibly truncated)."
        },
    )
    rope_scaling_mode: Optional[str] = field(default="dynamic")  # Llama 3 uses dynamic NTK scaling
    rope_scaling_factor: Optional[int] = field(default=2.0)
...
def train():
    ...
    config = transformers.AutoConfig.from_pretrained(
        ...
        rope_scaling={"type": "dynamic", "factor": 2.0}  # Correct format
    )
    model = transformers.AutoModelForCausalLM.from_config(config).to(
        device="cuda", 
        dtype=torch.bfloat16  # Llama 3 works better with bfloat16
    )
    ...
    for length in [512]: # [4096, 8192, 16384, 32768]:
        time_list = []
        data = {"input_ids": torch.randint(0, 32000, (1, length,), device="cuda"),
                "labels": torch.randint(0, 32000, (1, length,), device="cuda"), 
