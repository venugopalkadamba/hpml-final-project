...
def calculate_metrics(profiler_data):
    ...
    return {
        ...
        "Bandwidth (GiB per second)": bandwidth * 1e-9 if bandwidth else None,
    }
...
def train():
    ...
    #     if isinstance(rope_scaling, dict) and ("type" not in rope_scaling or "factor" not in rope_scaling):
    #         # Convert to the expected format
    #         config.rope_scaling = {
    #             "type": rope_scaling.get("rope_type", "linear"),  # Default to 'linear'
    #             "factor": rope_scaling.get("factor", 1.0)        # Default factor
    #         }
